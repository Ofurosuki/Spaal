...
def main():
    parser = argparse.ArgumentParser(description="Full pipeline: Generate, Denoise, and Reconstruct LiDAR data.")
    ...
    # Denoiser args
    parser.add_argument("--ckpt-path", type=str, required=True, help="Path to the denoiser model checkpoint file.")

    # Reconstructor args
    parser.add_argument("--output-pcd-dir", type=str, required=True, help="Path to the directory to save reconstructed .pcd files.")

    args = parser.parse_args()

    # --- 1. Data Generation ---
    print("--- Step 1: Generating LiDAR Data ---")
    generator = LidarSignalDatasetGenerator(
        lidar_type=args.lidar_type,
        pcd_directory=args.pcd_directory,
        # output_dir is not needed as we are not saving intermediate files
    ...
    denoised_signals = run_denoising(denoiser, generated_data['signals'])

    # --- 3. Reconstruction ---
    print("\n--- Step 3: Reconstructing Point Clouds ---")
