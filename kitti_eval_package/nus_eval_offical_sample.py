...
def main():
    ...
    parser.add_argument(
        "--sample-tokens",
        default=r"D:\eval_cvpr2026\minival_tokens.json",
    ...
    parser.add_argument(
        ...
        help="Evaluation config to use"
    )
    parser.add_argument(
        "--output-dir",
        default=r"D:\eval_cvpr2026\eval_output",
    ...
    if args.center_angle is not None and args.angle_width is not None:
        print(f"Angle Filter: center={args.center_angle}°, width={args.angle_width}° "
              f"(range: {args.center_angle - args.angle_width/2:.1f}° to {args.center_angle + args.angle_width/2:.1f}°)")
    print()
    ...
    print(f"Evaluation config: {args.config}")
    ...
    sample_tokens = load_sample_tokens(args.sample_tokens)
    print()
    ...
    pred_boxes = filter_eval_boxes(nusc, pred_boxes, cfg.class_range, verbose=args.verbose)
    print()

    print("Filtering ground truth by distance range...")
    gt_boxes = filter_eval_boxes(nusc, gt_boxes, cfg.class_range, verbose=args.verbose)
    print()

    # Filter boxes by angle range (if specified)
    if args.center_angle is not None and args.angle_width is not None:
        print("Filtering ground truth by angle range...")
        gt_boxes = filter_gt_boxes_by_angle(gt_boxes, nusc, args.center_angle, args.angle_width)
        print()
    ...
    details_file = os.path.join(args.output_dir, "metrics_details.json")
