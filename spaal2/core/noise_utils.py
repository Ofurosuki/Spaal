...
# This is a placeholder for the Echo class, as its definition is in another file.
# A proper implementation would import it.
...
def accumulate_signal_list(signal_list: list[npt.NDArray[np.float64]]) -> npt.NDArray[np.float64]:
    ...
    Args:
        signal_list (list[npt.NDArray[np.float64]]): 
            信号のリスト
...
def detect_echo(signal: npt.NDArray[np.float64], max_height: float = 9.0, thd_factor: float = 1.0, use_height_estimation: bool = False, pulse_half_width_ns: int = 10, show_plot: bool = False) -> list[Echo]:
    ...
    elif len(raises) == 0 or (len(falls) > 0 and raises[0] > falls[0]):
        raises = np.insert(raises, 0, 0)
    if len(falls) == 0 or (len(raises) > 0 and raises[-1] > falls[-1]):
