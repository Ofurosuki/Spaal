...
class PcdLidarHDL64E:
    ...
    def _read_pcd(self, file_path: str):
        ...
        if len(self.points) > 0:
            ...
            self.initial_azimuth_offset = np.rad2deg(np.arctan2(first_point[0], first_point[1]))
        else:
            self.initial_azimuth_offset = 0

        # NEW ARCHITECTURE: Use channel arrays instead of depth_map
    ...
    def _create_channel_arrays(self):
        """
        NEW ARCHITECTURE: Create channel-based arrays instead of depth_map.
        Each channel stores sorted points by azimuth, downsampled to max_samples_per_channel.
        Returns: (channel_arrays, max_samples_per_channel)
        """
        if len(self.points) == 0:
            return {}, 0

        points = self.points
        max_samples_per_channel = int(360 / self.output_horizontal_resolution_deg)  # 4400 for 0.0818°
        ...
        x, y, z = x[non_zero_mask], y[non_zero_mask], z[non_zero_mask]
        ...
        channel_indices = np.argmin(diffs, axis=1)

        # Get intensities
        intensities = self.intensities
        if intensities is not None:
            intensities = intensities[non_zero_mask]
        else:
        ...
        for ch_idx in range(64):
            ...
            ch_azimuths = azimuth_deg[mask]
            ch_distances = distances[mask]
            ch_intensities = intensities[mask]
            ch_original_indices = np.arange(len(self.points))[non_zero_mask][mask]

            # Sort by azimuth
            sort_idx = np.argsort(ch_azimuths)
            ch_azimuths = ch_azimuths[sort_idx]
            ch_distances = ch_distances[sort_idx]
            ch_intensities = ch_intensities[sort_idx]
            ch_original_indices = ch_original_indices[sort_idx]

            # Adjust to exactly max_samples_per_channel
            num_points = len(ch_azimuths)
            if num_points > max_samples_per_channel:
                # Evenly spaced downsampling
                indices = np.linspace(0, num_points - 1, max_samples_per_channel, dtype=int)
                ch_azimuths = ch_azimuths[indices]
                ch_distances = ch_distances[indices]
                ch_intensities = ch_intensities[indices]
                ch_original_indices = ch_original_indices[indices]
            elif num_points < max_samples_per_channel:
                # Pad with NaN/zeros to fill to max_samples_per_channel
                pad_size = max_samples_per_channel - num_points
                ch_azimuths = np.concatenate([ch_azimuths, np.full(pad_size, np.nan)])
                ch_distances = np.concatenate([ch_distances, np.zeros(pad_size)])
                ch_intensities = np.concatenate([ch_intensities, np.zeros(pad_size)])
                ch_original_indices = np.concatenate([ch_original_indices, np.full(pad_size, -1, dtype=int)])

            channel_arrays[ch_idx] = {
                'azimuths': ch_azimuths,
                'distances': ch_distances,
                'intensities': ch_intensities,
                'original_indices': ch_original_indices
            }

        total_points = sum(len(ch['azimuths']) for ch in channel_arrays.values())
        print(f"[DEBUG] Created channel arrays with {total_points} points from {len(self.points)} original points")
        print(f"[DEBUG] Max samples per channel: {max_samples_per_channel}")

        return channel_arrays, max_samples_per_channel
    ...
    def _create_depth_map(self):
        ...
        # Calculate azimuth from x, y coordinates (in degrees, -180 to 180)
        azimuth_deg = np.rad2deg(np.arctan2(x, y))
        ...
        # 3. INDEX-BASED key creation (no h_offset applied to avoid collisions)
        # Calculate azimuth step from initial_azimuth_offset
        # This creates a direct mapping to the scan grid
        normalized_azimuth_deg = (azimuth_deg - self.initial_azimuth_offset + 360) % 360

        # Use internal resolution (0.1°) for fine-grained depth map
        azimuth_steps = np.round(normalized_azimuth_deg / self.horizontal_resolution_deg).astype(int)
        azimuth_steps = azimuth_steps % int(360 / self.horizontal_resolution_deg)  # Wrap around

        # Channel indices
        channel_indices = closest_v_angle_indices

        # Keys are now (channel_idx, azimuth_step) instead of (azimuth_deg, altitude_deg)
        # This avoids collisions from h_offset
        azimuth_keys = azimuth_steps
        altitude_keys = channel_indices
        ...
        for i in range(len(final_azimuth_keys)):
            # INDEX-BASED: key = (channel_idx, azimuth_step)
            # altitude_keys = channel_indices, azimuth_keys = azimuth_steps
            key = (final_altitude_keys[i], final_azimuth_keys[i])
        ...
        print(f"[DEBUG] Created depth_map with {len(depth_map)} entries from {len(self.points)} original points")
    ...
    def new_frame_from_points(self, points: np.ndarray, intensities: np.ndarray, base_timestamp: PreciseDuration = PreciseDuration(nanoseconds=0)) -> "PcdLidarHDL64E":
        ...
        if len(self.points) > 0:
            first_point = self.points[0]
            # IMPORTANT: Must match arctan2(x, y) used in _create_depth_map()
            self.initial_azimuth_offset = np.rad2deg(np.arctan2(first_point[0], first_point[1]))
        else:
            self.initial_azimuth_offset = 0

        # NEW ARCHITECTURE: Create channel arrays
        self.channel_arrays, self.samples_per_channel = self._create_channel_arrays()

        # OLD: Create depth_map (will be deprecated)
        self.depth_map, self.original_point_indices_map = self._create_depth_map()
        self.no_signal_scan_angles: list[tuple[int, int]] = []
    ...
    def scan(self) -> tuple[MeasurementConfig, npt.NDArray[np.float64]]:
        if self.index >= self.max_index:
            raise StopIteration()
        azimuth_step_output, channel_idx = self._get_current_angle()
        timestamp = self._get_current_timestamp()

        signal_length = int(self.accept_window.in_nanoseconds / self.time_resolution_ns)
        signal = np.zeros((signal_length, ))

        # NEW ARCHITECTURE: Read directly from channel_arrays
        ch_data = self.channel_arrays[channel_idx]

        # Check if this azimuth_step_output has data in this channel
        has_data = False
        depth = None
        pcd_intensity = None
        actual_azimuth_deg = None

        if azimuth_step_output < len(ch_data['azimuths']):
            # Data exists for this index - check if it's a real point (not padding)
            actual_azimuth_deg = ch_data['azimuths'][azimuth_step_output]
            depth = ch_data['distances'][azimuth_step_output]
            pcd_intensity = ch_data['intensities'][azimuth_step_output]
            point_idx = ch_data['original_indices'][azimuth_step_output]

            # Skip padded entries (distance=0, azimuth=NaN, or index=-1)
            if depth > 0 and not np.isnan(actual_azimuth_deg) and point_idx >= 0:
                        ...
                        if start_idx < end_idx:
                            ...
                            if pcd_intensity is not None:
                                if pcd_intensity > 0:
                                    ...
                                    # intensity=0の場合、デフォルト値を使用（検出可能な最小値）
                                    pulse_amplitude = 0.05 * self.intensity_to_amplitude_ratio
        ...
        # For MeasurementConfig compatibility: use synthetic azimuth if no actual data
        if actual_azimuth_deg is None or np.isnan(actual_azimuth_deg):
            actual_azimuth_deg = azimuth_step_output * self.output_horizontal_resolution_deg + self.initial_azimuth_offset
        ...
        if not has_data:
            self.no_signal_scan_angles.append((azimuth_key, altitude_key))
        ...
        return MeasurementConfig(
            ...
            azimuth_deg=actual_azimuth_deg,
            horizontal_index=azimuth_step_output,
        ), signal
